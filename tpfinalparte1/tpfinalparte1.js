...
function dibujarPantalla() {
  ...
  for (let i = 0; i < cantLineas; i++) {
    if (estado * cantLineas + i < guion.length) {
      text(guion[estado * cantLineas + i], width / 2, height / 2 - 100 + i * 30);
    }
  }
  if (estado == 0) {
    dibujarBoton(240, 300, 150, 40, "Siguiente", true);
    dibujarBoton(240, 360, 150, 40, "Créditos", true);
  } else if (estado == 4 || estado == 11 || estado == 19 || estado == 22) {
    dibujarBoton(240, 300, 150, 40, "Volver a inicio", true);
  } else if (estado !== 18 && estado !== 3 && estado !== 6 && estado !== 10) {
    dibujarBoton(240, 300, 150, 40, "Siguiente", true);
  }
  ...
  if (estado == 7 || estado == 21 || estado == 14) {
    dibujarBoton(240, 300, 150, 40, "Siguiente", true);
  }
  push();
  textSize(40);
...
function mousePressed() {
  ...
  } else if (estado == 14 && botonPresionado(240, 300, 150, 40)) {
    avanzar(10);  
  } else if (estado == 21 && botonPresionado(240, 300, 150, 40)) {
    avanzar(11);  
